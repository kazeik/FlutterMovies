...
class MoviesHistoryView extends StatefulWidget {
  List<MovieDetailMac> movies;

  MoviesHistoryView({Key key, this.movies}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _MoviesHistoryView();
}

class _MoviesHistoryView extends State<MoviesHistoryView> {
  @override
  void initState() {
    super.initState();
    getFavListData();
          ...
          Stack(
            children: <Widget>[
              ...
              backgroundImage: CachedNetworkImageProvider(
                  'http://www.jinrikanpian.net/' + movie.vodPic),
              radius: 40.0,
            ),*/
              ),
              Positioned(
  ...
  Future<void> getFavListData() async {
    try {
      var _favListData = await MovieServices.getList('historyMovie');
  ...
  cancelHistory(movie) async {
    List moviesListData = await MovieServices.getList('historyMovie');
    try {
      ...
      Storage.setString('historyMovie', json.encode(moviesListData));
      getFavListData();
    } catch (e) {
      return [];
    }
